...
mixin SyncedPageControllersMixin<T extends StatefulWidget> on State<T> {
  ...
  void initializeSync() {
    _currentScrolling = ValueNotifier<SyncedPageViewType>(SyncedPageViewType.primary);
  }
  ...
  void startSync(PageController primaryController, PageController secondaryController) {
    primaryController.addListener(() => _primaryListener(primaryController, secondaryController));
    secondaryController.addListener(() => _secondaryListener(primaryController, secondaryController));
  }

  /// Stop syncing two PageControllers
  void stopSync(PageController primaryController, PageController secondaryController) {
    primaryController.removeListener(() => _primaryListener(primaryController, secondaryController));
    secondaryController.removeListener(() => _secondaryListener(primaryController, secondaryController));
  }

  void _primaryListener(PageController primaryController, PageController secondaryController) {
    ...
    final clampedOffsetPrimary = primaryController.offset.clamp(0.0, maxPrimary);
  ...
  void _secondaryListener(PageController primaryController, PageController secondaryController) {
    if (!primaryController.hasClients || !secondaryController.hasClients) {
      return;
    }
    ...
    final clampedOffsetSecondary = secondaryController.offset.clamp(0.0, maxSecondary);
